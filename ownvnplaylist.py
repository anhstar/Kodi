	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.anhstar.ownvnplaylist/(.+?)/.+?\://", item["path"]):
				match = re.search("plugin.video.anhstar.ownvnplaylist(/.+?/).+?\://", item["path"])
			...
			if "spreadsheets/d/" in item["path"]:
				# https://docs.google.com/spreadsheets/d/1PgEY4s66O1Y5Of9aqs_v_tU4FZAiYt0-c-o9fI98vZg/edit#gid=0
...
def AddTracking(items):
	...
	for item in items:
		if "plugin.video.anhstar.ownvnplaylist" in item["path"]:
